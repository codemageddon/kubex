...
@pytest.mark.anyio
async def test_status_get_builds_correct_request() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    result = await api.status.get("my-pod")
    req = client.last_request
    assert req.method == "GET"
    assert req.url.endswith("/pods/my-pod/status")
...
@pytest.mark.anyio
async def test_status_get_with_explicit_namespace() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    await api.status.get("my-pod", namespace="other")
    req = client.last_request
    assert "namespaces/other" in req.url


@pytest.mark.anyio
async def test_status_replace_builds_correct_request() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    pod = Pod.model_validate(
        {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "my-pod"}}
    )
    result = await api.status.replace("my-pod", pod)
    req = client.last_request
...
@pytest.mark.anyio
async def test_status_replace_with_dry_run() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    pod = Pod.model_validate(
        {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "my-pod"}}
    )
    await api.status.replace("my-pod", pod, dry_run=True)
    req = client.last_request
...
@pytest.mark.anyio
async def test_status_replace_with_field_manager() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    pod = Pod.model_validate(
        {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "my-pod"}}
    )
    await api.status.replace("my-pod", pod, field_manager="my-manager")
    req = client.last_request
    assert req.query_params is not None
...
@pytest.mark.anyio
async def test_status_patch_builds_correct_request() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    patch = MergePatch(Pod.model_validate({"metadata": {"labels": {"a": "b"}}}))
    result = await api.status.patch("my-pod", patch)
    req = client.last_request
...
@pytest.mark.anyio
async def test_status_patch_with_options() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client, namespace="default")
    patch = MergePatch(Pod.model_validate({"metadata": {"labels": {"a": "b"}}}))
...
@pytest.mark.anyio
async def test_status_get_requires_namespace_for_namespaced_resource() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client)
    with pytest.raises(ValueError, match="Namespace is required"):
        await api.status.get("my-pod")


@pytest.mark.anyio
async def test_status_replace_requires_namespace_for_namespaced_resource() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client)
    pod = Pod.model_validate(
        {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "my-pod"}}
    ...
    with pytest.raises(ValueError, match="Namespace is required"):
        await api.status.replace("my-pod", pod)


@pytest.mark.anyio
async def test_status_patch_requires_namespace_for_namespaced_resource() -> None:
    client = StubClient(response_content=POD_JSON)
    api: Api[Pod] = Api(Pod, client=client)
    patch = MergePatch(Pod.model_validate({"metadata": {"labels": {"a": "b"}}}))
    with pytest.raises(ValueError, match="Namespace is required"):
        await api.status.patch("my-pod", patch)


@pytest.mark.anyio
async def test_status_get_deserializes_response_to_resource_type() -> None:
    response_json = (
