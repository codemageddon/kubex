...
from kubex.k8s.v1_37.apiserverinternal.v1alpha1.storage_version_spec import (
    StorageVersionSpec,
)
from kubex.k8s.v1_37.apiserverinternal.v1alpha1.storage_version_status import (
...
class StorageVersion(ClusterScopedEntity, HasStatusSubresource):
    ...
    spec: StorageVersionSpec | None = Field(
        default=None,
        alias="spec",
        description="spec is an empty spec. It is here to comply with Kubernetes API style.",
    )
    status: StorageVersionStatus | None = Field(
        default=None,
        alias="status",
        description="status on the version the API server instance can decode from and encode objects to when persisting objects in the backend.",
    )
