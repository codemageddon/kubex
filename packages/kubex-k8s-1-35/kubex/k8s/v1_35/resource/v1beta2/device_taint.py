...
class DeviceTaint(BaseK8sModel):
    ...
    effect: str = Field(
        ...,
        alias="effect",
        description="The effect of the taint on claims that do not tolerate the taint and through such claims on the pods using them. Valid effects are None, NoSchedule and NoExecute. PreferNoSchedule as used for nodes is not valid here. More effects may get added in the future. Consumers must treat unknown effects like None.",
    )
    key: str = Field(
        ...,
        alias="key",
