from kubex.k8s.v1_36.resource.v1.device_toleration import DeviceToleration
...
class DeviceRequestAllocationResult(BaseK8sModel):
    ...
    admin_access: bool | None = Field(
        default=None,
        alias="adminAccess",
        description="AdminAccess indicates that this device was allocated for administrative access. See the corresponding request field for a definition of mode. Admin access is disabled if this field is unset or set to false, otherwise it is enabled.",
    )
    binding_conditions: list[str] | None = Field(
        default=None,
        alias="bindingConditions",
        description="BindingConditions contains a copy of the BindingConditions from the corresponding ResourceSlice at the time of allocation. This is a beta field and requires enabling the DRADeviceBindingConditions and DRAResourceClaimDeviceStatus feature gates.",
    )
    binding_failure_conditions: list[str] | None = Field(
        default=None,
        alias="bindingFailureConditions",
        description="BindingFailureConditions contains a copy of the BindingFailureConditions from the corresponding ResourceSlice at the time of allocation. This is a beta field and requires enabling the DRADeviceBindingConditions and DRAResourceClaimDeviceStatus feature gates.",
    )
    ...
    tolerations: list[DeviceToleration] | None = Field(
        default=None,
        alias="tolerations",
        description="A copy of all tolerations specified in the request at the time when the device got allocated. The maximum number of tolerations is 16. This is a beta field and requires enabling the DRADeviceTaints feature gate.",
    )
