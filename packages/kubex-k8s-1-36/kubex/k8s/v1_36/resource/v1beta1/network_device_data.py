...
class NetworkDeviceData(BaseK8sModel):
    ...
    hardware_address: str | None = Field(
        ...
        description="HardwareAddress represents the hardware address (e.g. MAC Address) of the device's network interface. Must not be longer than 128 bytes.",
    )
    interface_name: str | None = Field(
        default=None,
        alias="interfaceName",
        description="InterfaceName specifies the name of the network interface associated with the allocated device. This might be the name of a physical or virtual network interface being configured in the pod. Must not be longer than 256 bytes.",
    )
